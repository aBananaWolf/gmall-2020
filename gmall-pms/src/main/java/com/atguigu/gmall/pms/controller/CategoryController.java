...
public class CategoryController {
    ...
    @GetMapping
    /**
     * 分类查询，前台默认传参0，查询所有的分类，前端维护分类
     */
    public Resp<List<CategoryEntity>> category(@RequestParam(value = "level", required = false) Integer level,
                                               @RequestParam(value = "parentCid",required = false) Long parentCid) {

        QueryWrapper<CategoryEntity> categoryEntityQueryWrapper = new QueryWrapper<>();
        if (level != null && level != 0){
            categoryEntityQueryWrapper.eq("cat_level", level);
        }
        if (parentCid != null) {
            categoryEntityQueryWrapper.eq("parent_cid", parentCid);
        }
        List<CategoryEntity> list = categoryService.list(categoryEntityQueryWrapper);
        return Resp.ok(list);
    }
